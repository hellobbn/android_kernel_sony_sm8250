					...
					struct find_best_target_env *fbt_env)
{
	...
	for (cluster = 0; cluster < num_sched_clusters; cluster++) {
		...
		cpumask_and(&visit_cpus, &p->cpus_allowed,
				     ...
				     int sync, int sibling_count_hint)
{
	...
	if (cpumask_test_cpu(prev_cpu, &p->cpus_allowed) && !__cpu_overutilized(prev_cpu, delta)) {
		if (trace_walt_sched_compute_energy_enabled()) {
		...
		trace_walt_sched_compute_energy(p, prev_cpu, prev_energy, 0, 0, 0, &output);
	} else {
	...
	for_each_cpu(cpu, candidates) {
		...
		trace_walt_sched_compute_energy(p, cpu, cur_energy,
